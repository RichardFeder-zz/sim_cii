...
class sphx_test_916():
    ...
    def Ipred_targ_redshift_covariance(self, Ipred_targ_arr, line_name_array, f_arr_pred=None, ilc_idx=None, nsigth=5, return_fig=True, zoom=True, cov_or_corr='corr'):
        
        
        f_arr_pred = np.array(f_arr_pred)
        sum_covariances, list_of_template_idxs = [], []

        big_data_matrix = np.zeros((Ipred_targ_arr.shape[0], (Ipred_targ_arr.shape[1]-1)*Ipred_targ_arr.shape[-1]))
    
        for line_idx in range(Ipred_targ_arr.shape[1]-1):
            for n in range(Ipred_targ_arr.shape[0]):
                
                big_data_matrix[n,line_idx*Ipred_targ_arr.shape[-1]:(line_idx+1)*Ipred_targ_arr.shape[-1]] = Ipred_targ_arr[n, line_idx]
                
        big_covariance_mat = np.cov(big_data_matrix.transpose())
                
        big_line_redshift_idxs = None
        
        for j in range(big_covariance_mat.shape[0]):
            for n, idxs in enumerate(np.nonzero(big_covariance_mat[j])):
                if len(idxs) > 1:
                    big_line_redshift_idxs = idxs
                    break

        nothin_counter = False
        list_of_line_redshift_idxs = []
        line_idxs = []
        for line_idx in range(Ipred_targ_arr.shape[1]-1):
            ...
            if len(Ipred_targ_arr.shape)==3:
                ...
                for ilc in range(Ipred_targ_arr.shape[2]):
                    ...
                    cov_mats.append(np.cov(data_matrix.transpose()))
                
                covariance_mat = np.mean(np.array(cov_mats), axis=0)
            ...
            line_redshift_idxs = None
            # if there are non-zero entries in the covariance matrix,
            if len(list_of_covariance_mats) > 0:
                # go through each row of the covariance matrix, the first row with non-zero entries will have the line redshift
                # idxs needed. set nothin_counter to True and move on from the loop
                for j in range(sum_covariance.shape[0]):
                    for n, idxs in enumerate(np.nonzero(sum_covariance[j])):
                        if len(idxs) > 1:
                            
                            line_redshift_idxs = idxs
                            nothin_counter = True
                            break
                            
                if line_redshift_idxs is not None:
                    list_of_line_redshift_idxs.append(line_redshift_idxs)
                    line_idxs.append(line_idx)

                if line_redshift_idxs is not None and zoom:
                    
                    sum_cov_zoom = sum_covariance[line_redshift_idxs]
                    sum_cov_zoom = sum_cov_zoom[:, line_redshift_idxs]
                    if cov_or_corr == 'corr':
                        ...
                        plt.imshow(sum_cov_zoom, vmin=-0.8*np.max(sum_cov_zoom), vmax=0.8*np.max(sum_cov_zoom), origin='lower', cmap='bwr')
                    plt.xticks(np.arange(len(line_redshift_idxs)), np.round(zbins[line_redshift_idxs], 2))
                    plt.yticks(np.arange(len(line_redshift_idxs)), np.round(zbins[line_redshift_idxs], 2))

                else:
                    if cov_or_corr == 'corr':
                        sum_correlation = np.array([[sum_covariance[i,j]/np.sqrt(sum_covariance[i,i]*sum_covariance[j,j]) for i in range(sum_covariance.shape[0])] for j in range(sum_covariance.shape[1])])
                        plt.imshow(sum_correlation, vmin=-1, vmax=1, origin='lower', cmap='bwr')
                    elif cov_or_corr == 'cov':
                        plt.imshow(sum_covariance, vmin=np.mean(sum_covariance), vmax=np.max(sum_covariance), origin='lower', cmap='bwr')
                ...
                if cov_or_corr=='cov':
                    cbar.set_label('[Jy$^2$/sr$^2$]', fontsize=16)
            list_of_template_idxs.append(line_redshift_idxs)


        if not nothin_counter:
            print('No lines found in this line of sight, exiting')
            plt.close()
            fig = None
        else:
            plt.tight_layout()
        ...
        big_fig = None
        if big_line_redshift_idxs is not None and nothin_counter:
            zoom_big_covariance_matrix = big_covariance_mat[big_line_redshift_idxs]
            zoom_big_cov_matrix = zoom_big_covariance_matrix[:,big_line_redshift_idxs]
            zoom_big_corr_matrix = np.array([[zoom_big_cov_matrix[i,j]/np.sqrt(zoom_big_cov_matrix[i,i]*zoom_big_cov_matrix[j,j]) for i in range(zoom_big_cov_matrix.shape[0])] for j in range(zoom_big_cov_matrix.shape[1])])
            big_fig = plt.figure(figsize=(12,13))
            plt.title('$\\rho(I_{z_i},I_{z_j})$, '+str(nsigth)+'$\\sigma$ stopping threshold', fontsize=32)
            ...
            for i, line_idx in enumerate(line_idxs):
                ...
                redidxs = list_of_line_redshift_idxs[i]
                
                zbin_ticklabels.extend(np.round(zbins[redidxs], 2))

                
                hlineidx += len(redidxs)
                plt.text(hlineidx-0.5*len(redidxs)-1, -3, self.line_name_arr[line_idx], fontsize=18)
                plt.text(-5, hlineidx-0.5*len(redidxs), self.line_name_arr[line_idx], fontsize=18)

                
 
                print('hline idx is ', hlineidx)
                plt.axvline(hlineidx, color='k', linewidth=2)
                plt.axhline(hlineidx, color='k', linewidth=2)
            
            plt.xticks(np.arange(len(zbin_ticklabels)), zbin_ticklabels)
            plt.yticks(np.arange(len(zbin_ticklabels)), zbin_ticklabels)
            plt.imshow(zoom_big_corr_matrix, vmin=-1, vmax=1, origin='lower', cmap='bwr')
            cbar = plt.colorbar(orientation='horizontal')
            plt.tight_layout()
            plt.show()
...
#             markers = ['o', '*', 'v', 'x', '+', '^', 'v', 'x', '+', '^']
            ...
            for line_idx in range(Ipred_targ_arr.shape[1]):
                ...
                if line_idx < 4:
#                     axes[id1][id2].plot(xval, np.mean(Ipred_targ_arr[:,line_idx,:], axis=0), linewidth=1, color=colors[line_idx])
#                     axes[id1][id2].fill_between(xval, np.percentile(Ipred_targ_arr[:,line_idx,:], 16, axis=0),np.percentile(Ipred_targ_arr[:,line_idx,:], 84, axis=0), color=colors[line_idx], alpha=0.5)
                    axes[id1][id2].plot(zbins, np.mean(Ipred_targ_arr[:,line_idx,:], axis=0), linewidth=1, color=colors[line_idx])
                    axes[id1][id2].fill_between(zbins, np.percentile(Ipred_targ_arr[:,line_idx,:], 16, axis=0),np.percentile(Ipred_targ_arr[:,line_idx,:], 84, axis=0), color=colors[line_idx], alpha=0.5)

                else:
                    axes[id1][id2].plot(xval, np.mean(Ipred_targ_arr[:,line_idx,:], axis=0), linewidth=1, color=colors[line_idx])
                    axes[id1][id2].fill_between(xval, np.percentile(Ipred_targ_arr[:,line_idx,:], 16, axis=0),np.percentile(Ipred_targ_arr[:,line_idx,:], 84, axis=0), color=colors[line_idx], alpha=0.5)
#                 axes[id1][id2].text(0.75*np.max(zbins), 0.75*np.max(Ipred_targ_arr[:,line_idx,:]), self.line_name_arr[line_idx], ha='center', va='top',  fontsize=24)
                axes[id1][id2].text(0.75*np.max(zbins), 0.75*np.max(np.percentile(Ipred_targ_arr[:,line_idx,:], 84, axis=0)), self.line_name_arr[line_idx], ha='center', va='top',  fontsize=24)
#                 axes[id1][id2].text(0.75*np.max(xval), 0.75*np.max(Ipred_targ_arr[:,line_idx,:]), self.line_name_arr[line_idx], ha='center', va='top',  fontsize=24)
                axes[id1][id2].set_xlabel('$z$', fontsize=14)
                
                zbins = self.nu0_arr[line_idx] / self.sphx_par.nu_bins - 1
                

                xv = np.arange(0, Ipred_targ_arr.shape[-1], 20)

                axes[id1][id2].set_ylabel('$I$ [Jy/sr]', fontsize=16)
                

            if ilc_idx is not None:
                id1 = -1
                id2 = -1
                xval = np.arange(len(xval))
                axes[id1][id2].plot(xval, self.Itrue_all[ilc_idx], color='k', label='truth', linewidth=2)
                axes[id1][id2].plot(xval, self.Itrue_all[ilc_idx]+self.noise[ilc_idx], color='g', linestyle='dashed', label='observed', linewidth=0.8)

                axes[id1][id2].plot(xval, np.median(f_arr_pred, axis=0), color='b', label='predicted')
                axes[id1][id2].fill_between(xval, np.percentile(f_arr_pred, 16, axis=0), np.percentile(f_arr_pred, 84, axis=0), color='b', alpha=0.2)

                axes[id1][id2].legend(fontsize=10, loc='best')
                axes[id1][id2].set_ylim(1.2*np.min(self.noise[ilc_idx]), 1.4*np.max(self.Itrue_all[ilc_idx]))
                axes[id1][id2].set_xlabel('$\\nu$ index', fontsize=16)
                axes[id1][id2].set_ylabel('$I$ [Jy/sr]', fontsize=16)
        ...
        if return_fig:
            return fig, f, sum_covariances, big_fig
